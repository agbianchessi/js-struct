...
function intField(p, offset, length, le, signed) {
    ...
    var 
     nativeSuff = (signed?'':'U') + 'Int' + (length * 8) + (le?'LE':'BE'),
        readMethod = Uint8Array.prototype['read' + nativeSuff], writeMethod = Uint8Array.prototype['write' + nativeSuff];
...
function charField(p, offset, length, encoding, secure) {
    ...
    self.get = function () {
        ...
        var result = new TextDecoder().decode( (p.buf.slice(offset, (offset + length))) );
...
function Struct() {
    ...
    this.allocate = function () {
        applyClosures(priv);
        if (Uint8Array.alloc) {
            priv.buf = Uint8Array.alloc(priv.len);
        } else {
            priv.buf = new Uint8Array(priv.len);
            priv.buf.fill(0);
        }
        allocateFields();
        priv.allocated = true;
    ...
    this.set = function (key, val) {
        if (arguments.length == 2) {
            if (key in priv.fields) {
                ...
                throw new Error('Can not find field ' + key);
        } else if (Object.prototype.toString.call(key) === "[object Uint8Array]") {
            this._setBuff(key);
        } else {
            for (var k in key) {
                this.set(k, key[k]);
